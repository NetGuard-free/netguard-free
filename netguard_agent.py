...
def _run_wizard():
    ...
    print("\n" + "═"*60)
    print("  NetGuard — Pierwsze uruchomienie")
...
class AIAnalyst:
    ...
    def analyze(self, context: str, question: str) -> str:
        ...
        system_prompt = """Jesteś NetGuard — lokalnym agentem bezpieczeństwa sieci domowej.
...
class AlertManager:
    ...
    def _send_daily_report(self, scanner, now_local):
        ...
        html = (
            ...
            '<h1 style="color:#00d4ff;margin:0;font-size:20px;">🛡 NetGuard — Raport dzienny</h1>'
            ...
            'NetGuard v1.0 | Raport wygenerowany lokalnie | Żadne dane nie opuściły Twojej sieci'
            '</div></body></html>'
        )
        # Tekst plain-text jako fallback
        plain = f"""NetGuard — Raport dzienny {date_str}
{'='*50}
...
def start_dashboard(scanner: 'NetworkScanner', analyzer: 'PacketAnalyzer',
    ...
    def favicon():
        for d in [script_dir, _os.path.dirname(_os.path.abspath(__file__)), _os.path.join(_os.path.expanduser('~'), 'netguard')]:
            candidate = _os.path.join(d, 'favicon.ico')
            if _os.path.exists(candidate):
                return send_from_directory(d, 'favicon.ico', mimetype='image/x-icon')
        return '', 404
...
class NetGuardAgent:
    ...
    def run(self, with_dashboard: bool = False):
        ...
        cprint("OK", f"NetGuard uruchomiony", f"Sieć: {CONFIG['network_range']} | Interfejs: {iface}")
        if IS_HOME:
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='NetGuard — Lokalny Agent Sieci Domowej')
    ...
    cprint("INFO", "Uruchamianie NetGuard...", f"Sieć: {CONFIG['network_range']}")
    agent = NetGuardAgent()
